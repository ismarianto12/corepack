...
class PhareSpase
{
    ...
    // function 
    public static function app_config($par)
    {

        // app_key = BjOQXNeikH0peIf3o8poFODMfdkZAW2O	
        // dynamic_key = UsECJEQLs8wLqBEhDWE6yKly6SHy3jJv
        // APP_ID = APP007
        if ($par == 'app_id') {
            $val = 'APP007';
        } else if ($par == 'dynamic_key') {
            $val = 'UsECJEQLs8wLqBEhDWE6yKly6SHy3jJv';
        } else if ($par == 'app_key') {
            $val = 'BjOQXNeikH0peIf3o8poFODMfdkZAW2O';
        }
        return $val;
    }

    public static function hubungan()
    {
        return [
            1 => 'Diri Sendiri',
            2 => 'Pasangan ',
    ...
    public static function createtazamid()
    {
        $data = \DB::table('tmparameter')->select(\DB::raw('max(id) as idtazam'))->first();
        return ($data->idtazam > 0 || $data->idtazam  !=  null) ? $data->idtazam + 1 : 1;
    }


    public static function create_bulan($par)
    {
        $a =  [
            1 => 'January',
            2 => 'Februari',
    ...
    public static function status_slik()
    {
        return [
            1 => 'Otomatis',
            2 => 'Tidak Perlu'
    ...
    public static function jens_hadiah()
    {
        return [
            1 => 'Haji',
            2 => 'Lainya'
    ...
    public  static function formatuang($nilai)
    {
        if ($nilai < 0) {
            return 0;
        } else {
            $data = str_replace(',', '', $nilai);
            return number_format($data, 0, 0, '.');
        }
    }

    public static function load_js($js)
    {
        $html = '';
        if (is_array($js)) {
            foreach ($js as $jss) {
                $html .= '<script src="' . $jss . '"></script>';
            }
        } else {
    ...
    public static function load_css($css)
    {
        $html = '';
        if (is_array($css)) {
            foreach ($css as $cs) {
                $html .= '<link href="' . $cs . '" rel="stylesheet">';
            }
        } else {
